...
public abstract class CollectableEffect : MonoBehaviourPun {
	...
	public void EventHit(GameObject car) {
		if (!executed) {
			...
			photonView.RPC ("ResetSpawn", RpcTarget.MasterClient);
			...
			if (executionTarget == ExecutionTarget.Player) {
				photonView.RPC ("Finish", RpcTarget.Others);
				Hit (car);
			
			// if target is not 'player' execut Hit on the enemies' instances and destroy this instance
			} else {
				photonView.RPC ("PreHitOnEnemies", RpcTarget.Others);
				Finish ();
			}
		}
	}
	...
	public void PreHitOnEnemies() {
		...
		Hit(PUNRaceManagerScript.instance.localCar.gameObject);
	}
