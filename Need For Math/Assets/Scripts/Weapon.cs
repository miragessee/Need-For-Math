...
public class Weapon : UnityEngine.MonoBehaviour {
    ...
    public Sprite lapPanelSprite;

	public void Start() {
		weaponManager = GetComponentInParent<WeaponManager> ();

        skillGUI = GameObject.Find("SkillGUI");
    }

	void Update () {
		// do not process if not on local player's computer
		if (weaponManager.GetComponent<PhotonView> ().Owner != PhotonNetwork.LocalPlayer)
			return;
		...
		if (isSelected && currentTime <= 0 && currentAmmo > 0 && (CrossPlatformInputManager.GetButtonDown ("Fire1") || CrossPlatformInputManager.GetAxis("Fire1") > 0)) {
            ...
            if (currentAmmo <= 0)
            {
                skillGUI.GetComponent<Image>().sprite = lapPanelSprite;
            }
